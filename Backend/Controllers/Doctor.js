...
exports.createProfile = asyncHandler(async (req, res) => {
  ...
  const { first_name, last_name, email, bio, experience } = personal_details;

  const { name, city, state, postal_code, contact_info, appointment_fee } =
    hospital_details;

  const { reg_number, reg_year, state_medical_council } = verification_details;
  ...
  if (!specializedDiseases || specializedDiseases.length <= 0) {
    res.status(404);
    throw new Error(
      "Atleast one disease, the doctor is specialized at is required"
    );
  }
  ...
  for (certificate of certifications) {
    ...
    if (!supportedFiletypes.includes(fileType)) {
      res.status(400);
      throw new Error(
        "File type is not supported. Type should be png, jpg, jpeg or pdf"
      );
    }

    const document = await uploadImageToCloudinary(
      certificate,
  ...
  const diseasesArr = await Promise.all(
    specializedDiseases.map(async (disease) => {
...
exports.getDoctorDetails = asyncHandler(async (req, res) => {
  ...
  const diseasesNames = await Promise.all(
    doctorDetails?.specializedDiseases?.map((disease) => {
  ...
  const doctor = {
    ...
    verification_details: {
      ...
      reg_year,
      state_medical_council:
        doctorDetails.verificationDetails.state_medical_council,
    },
...
//@desc- to get doctors info for appointment booking page
...
exports.getDoctorsInfo = asyncHandler(async (req, res) => {
  ...
  const data = {
    ...
    bio: userDetails?.doctorsProfile?.bio,
    hospital_details: userDetails?.doctorsProfile?.hospitalDetails,
    verified: userDetails?.doctorsProfile?.verified,
